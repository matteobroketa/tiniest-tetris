...
class _Screen:
 def __init__(self,seed=0):
  ...
  if self.mode=='proof':
   ...
   if not isinstance(self.keys,list):self.keys=list(self.keys)
  else:
   self.keys=[]
 ...
 def addstr(self,x,y,frame):
  ...
  filled=frame.count('#')
  if self.previous>=0 and filled<self.previous:self.clear_events+=1
  self.previous=filled;self.frames+=1
  if self.frames>10000:raise AssertionError('frame limit')
  for char in frame:self.checksum=(self.checksum*1000003+ord(char))%4294967291
 def getch(self):
  if self.mode=='proof':
   if self.index<len(self.keys):
    key=self.keys[self.index];self.index+=1;return key
   return -1
  self.state=(1103515245*self.state+12345)%2147483648;self.index+=1
  return (-1,97,98,99)[self.state>>16&3]

def wrapper(function):
 if os.environ.get('TETRIS_MODE')=='random-suite':
  total=clears=0;digest=1
  for seed in range(1,33):
   screen=_Screen(seed);function(screen);total+=screen.frames;clears+=screen.clear_events;digest=(digest*1000003+screen.checksum)%4294967291
  if (total,clears,digest)!=(5298,1,2583409795):raise AssertionError('random suite mismatch')
 ...
 if screen.mode=='proof':
  if (screen.frames,screen.clear_events,screen.checksum,screen.index)!=(477,6,4287608623,400):
